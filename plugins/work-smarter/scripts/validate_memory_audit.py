#!/usr/bin/env python3
"""Validate a staged memory audit against real before/after Markdown files."""
...
class AuditError(ValueError):
    """Raised when an audit ledger does not prove a safe staged change."""
...
def _validate_file_inventory(root: Path, supplied: Any, label: str) -> None:
    ...
    actual = markdown_inventory(root)
    _require(supplied == actual, f"{label}_files does not match the complete Markdown file inventory")
...
def _index_items(root: Path, supplied: Any, label: str) -> dict[str, dict[str, Any]]:
    ...
    for item in supplied:
        ...
        if kind == "pointer":
            ...
            target_contents = _safe_file(root, target_path).read_text(encoding="utf-8")
            _require(target_text in target_contents, f"Broken pointer target for {item_id}: {target_path}")
        indexed[item_id] = item
    return indexed


def validate_audit(before_root: Path, after_root: Path, ledger: dict[str, Any]) -> None:
    """Validate a complete, bidirectional semantic ledger without changing either root."""
    ...
    for decision in decisions:
        ...
        if disposition in {"retained", "moved"}:
            ...
            _require(any(target["kind"] == source["kind"] and target["text"] == source["text"] for target in targets),
            ...
            _require(decision.get("semantic_review") == "passed", f"Reworded item lacks passed semantic review: {before_id}")
            ...
            _require(decision.get("authorized") is True, f"Updated item lacks explicit authorization: {before_id}")
            ...
            _require(any(target["kind"] == "historical_item" and target["text"] == source["text"] for target in targets),
    ...
    _require(referenced_after | set(new_after) == set(after),
             "Every after item must be linked to before state or explicitly marked new")
    if new_after:
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("PASS memory audit: complete file inventories and bidirectional item ledger validated")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"FAIL memory audit: {exc}", file=sys.stderr)
        raise SystemExit(1)
